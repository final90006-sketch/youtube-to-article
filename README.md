...
- **每日回顧 `review.html`**：掃全庫文章的「🧠 自我檢核」卡，用間隔重複（半衰期 7／14／28 天、「記得／忘了」二元回饋、進度存瀏覽器 localStorage）幫你複習；知識總覽首頁有「🧠 每日回顧」入口鈕，桌面 GUI 收尾時也會自動更新。
- 全部歸進 `桌面\YT影片文章\` 的可搜尋知識總覽首頁。
- **不只影片**：本機 **PDF／網頁文章／Markdown／Word** 也能進同一條管線（`fetch_document.py`），支援多檔合併（`--merge`）與敏感內容標記（`--private`：不進知識庫、不外送）。多檔合併時正文以【來源①②③】分節，閱讀版把 `（來源N）` 標成可點錨點、跳到文末對應來源的原文附錄，方便回溯出處。同樣產出精讀閱讀版與衍生卡片。
...
- `pymupdf`：**要把 PDF 轉成文章才需要**——沒裝就貼 PDF 會提示 `NEEDS_DEP`，裝了才讀得到（`python -m pip install -U pymupdf`）
- `trafilatura`（選配）：抓「網頁文章」時裝了正文擷取品質更好；沒裝會走內建退路、仍可用
...
> **想換知識庫位置？** 預設在 `桌面\YT影片文章\`。在技能資料夾放一個 `base_path.txt`、裡面寫一行你要的資料夾絕對路徑即可（沒有這個檔就用預設）；四支腳本與 GUI 都會改讀它。
