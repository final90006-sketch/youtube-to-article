...
python "<技能資料夾>\scripts\derive_extras.py" "<夾>"
...
- [ ] （機械複查）跑 `py -3 scripts\lint_article.py "<夾>"` 做零 LLM 品質門自檢（佔位搪塞語／份量樓地板／時間碼單調性／各章時間覆蓋）；只報不擋，有 WARN 就回頭補。
...
- `scripts/render_html.py` —— 自帶極簡 Markdown→HTML；navy/gold 大字宋體閱讀版，時間碼可點回原片（對不上逐字稿真實段落的假時間碼自動降級為不可跳，防跳錯），含列印樣式。
- `scripts/derive_extras.py` —— 離線零 LLM 從 article.md 萃取三份衍生單檔：`digest.html`（一頁精華卡·可列印）／`mindmap.html`（章節心智圖）／`quiz.html`（自測翻牌卡），放同夾；第 3 步已納入預設流程。
...
- `scripts/lint_article.py` —— 對已寫好的 `article.md` 跑零 LLM 機械品質門（佔位搪塞語黑名單／份量樓地板／時間碼單調性／各章時間覆蓋；判定閾值抄自 GUI 的產後審計，兩者同標準）：`py -3 scripts/lint_article.py "<夾>"`，只報不擋（exit 0）。對話版寫完可自檢，不必依賴 GUI。
